...
%     Dxxxx(2, 2) = 7 / h^4;
%     Dxxxx(end-1, end-1) = 7 / h^4;
%     Dxxxx(1, 1:3) = [6, -8, 2] / h^4;
%     Dxxxx(end, end-2:end) = [2, -8, 6] / h^4;
    
    Dxxxx(2, 1:4) = [-4, 7, -4, 1] / h^4;
    Dxxxx(1,1:3) = [6, -8, 1] / h^4;
    Dxxxx(end-1, end-3:end) = [1, -4, 7, -4] / h^4;
    Dxxxx(end,end-2:end) = [2, -8, 6] / h^4;
elseif bc == "s"
...
%             + (lambdaSq + 4 * muSq + 2 * sig1 * k / h^2) * (uVec(2) + uVirt1) ...
%             - muSq * (uVec(3) + uVirt2) + (-1 + sig0 * k + 4 * sig1 * k / h^2) * uPrevVec(1) ...
%             - 2 * sig1 * k / h^2 * (uPrevVec(2) + uPrevVirt1)) / (1+sig0 * k);
%         
%         uNextVec(end) = ((2 - 2 * lambdaSq - 6 * muSq - 4 * sig1 * k / h^2) * uVec(end)...
%             + (lambdaSq + 4 * muSq + 2 * sig1 * k / h^2) * (uVirtN1 + uVec(end-1)) ...
%             - muSq * (uVirtN2 + uVec(end-2)) + (-1 + sig0 * k + 4 * sig1 * k / h^2) * uPrevVec(end) ...
%             - 2 * sig1 * k / h^2 * (uPrevVirtN1 + uPrevVec(end-1))) / (1+sig0 * k);
%         
% %         uNextVec(1) = ((2 - 2 * lambdaSq - 6 * muSq - 4 * sig1 * k / h^2) * uVec(1)...
...
%          
%         potEnergy(n) = potEnergy(n) + 1/2 * E * I * h / 2 * (1/h^2 * (2 * u(2) - 2 * u(1))) ...
%              .* (1/h^2 * (2 * uPrev(2) - 2 * uPrev(1)));
%         potEnergy(n) = potEnergy(n) + 1/2 * E * I * h / 2 * (1/h^2 * (2 * u(end-1) - 2 * u(end))) ...
%              .* (1/h^2 * (2 * uPrev(end-1) - 2 * uPrev(end)));
%          
