...
% else
%     plot(out, 'b', 'Linewidth', 2)
% end
%% 
figure('Position', [440 576 357 222])
t = (0:2000) / fs;
plot(t, out(1:length(t)), 'k', 'Linewidth', 2)
xlim([0, t(end)])
ylim([-1.2, 1.2])
xLab = xlabel('$t$', 'interpreter', 'latex')
yLab = ylabel('$u(t)$', 'interpreter', 'latex')
xLim = xlim;
yLim = ylim;
xLab.Position(2) = yLim(1) - 0.10 * (yLim(2) - yLim(1));
yLab.Position(1) = xLim(1) - 0.07 * (xLim(2) - xLim(1));
set(gca, 'Fontsize', 16, 'tickLabelInterpreter', 'latex', ...
    'Position', [0.1232 0.1667 0.8554 0.8035], 'Linewidth', 2)
